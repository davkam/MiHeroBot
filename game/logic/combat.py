...
class Combat():
    ...
    async def pre_combat(self):
        ...
        if type(self.objB.character) == Player:
            ...
            self.log.append([f"WEAPON: {objA.weapon.weapon_class.name.upper()} LVL.{objA.weapon.attack.get_lvl()}", f"ARMOR: {objA.armor.armor_class.name.upper()} LVL.{objA.armor.defense.get_lvl()}"])
            self.log.append([f"WEAPON: {objB.weapon.weapon_class.name.upper()} LVL.{objB.weapon.attack.get_lvl()}", f"ARMOR: {objB.armor.armor_class.name.upper()} LVL.{objB.armor.defense.get_lvl()}"])
        else:
            objA: Player = self.objA.character
        ...
        while self.objA.hp > 0 and self.objB.hp > 0:
            if turn == True:
                ...
                if rng <= self.objA.acc:
                    ...
                    hit_log = await Features.center_string(string = f"CRITICAL HIT: {dmg}", max_chars = 28)
                else:
                    ...
                    hit_log = await Features.center_string(string = f"BLUNT HIT: {dmg}", max_chars = 28)
    ...
    async def post_combat(self):
        ...
        if self.objA.hp <= 0:
            if type(self.objB.character) == Player:
                ...
                self.log += [pvp_deadA]
                w_name = await Features.center_string(string = winner.name, max_chars = 28)
                l_name = await Features.center_string(string = loser.name, max_chars = 28)
