...
class StatsRenderer(Renderer):
    ...
    async def render_stats(self) -> str:
        ...
        player_stats = await self.render_player_stats()
        ...
        return await TempImageHandler.new_temp_image(image=image)

    async def render_char(self) -> Image.Image:
        image = Image.new(mode="RGBA", size=(264, 256), color=(0, 0, 0, 0))

        char = Image.open(fp=StatsRenderer.CHAR_PATH % str(self.user.player.color.value))

        if self.user.player.equipment.sword:
            sword = Image.open(fp=StatsRenderer.SWORD_PATH % str(self.user.player.equipment.sword.tier.value))
            image.paste(im=sword, box=(148, 8), mask=sword)

        image.paste(im=char, box=(-16, 0), mask=char)

        if self.user.player.equipment.head:
            head = Image.open(fp=StatsRenderer.HEAD_PATH % str(self.user.player.equipment.head.tier.value))
            image.paste(im=head, box=(-16, 8), mask=head)

        if self.user.player.equipment.body:
            body = Image.open(fp=StatsRenderer.BODY_PATH % str(self.user.player.equipment.body.tier.value))
            image.paste(im=body, box=(-16, 0), mask=body)

        if self.user.player.equipment.shield:
            shield = Image.open(fp=StatsRenderer.SHIELD_PATH % str(self.user.player.equipment.shield.tier.value))
            image.paste(im=shield, box=(-12, 124), mask=shield)

        return image
    
    async def render_name(self) -> Image.Image: # 256, 64
    ...
    async def render_equips(self) -> Image.Image: # 256, 160
        ...
        if self.user.player.equipment.amulet:
            amulet = Image.open(fp=StatsRenderer.AMULET_PATH % str(self.user.player.equipment.amulet.tier.value))
    ...
    async def render_player_stats(self) -> Image.Image: # 288, 144
        ...
        level_bar = await Bar.get_bar(act_val=level_prog, max_val=100)
        draw.text(xy=(24, 8), text="LEVEL", fill=(255, 255, 255), font=font, stroke_width=1, stroke_fill=(0, 0, 0))
        draw.text(xy=(120, 8), text=str(level), fill=(0, 255, 0), font=font, stroke_width=1, stroke_fill=(0, 0, 0))
        ...
        attack_bar = await Bar.get_bar(act_val=attack_prog, max_val=100)                     
        draw.text(xy=(24, 40), text="ATTACK", fill=(255, 255, 255), font=font, stroke_width=1, stroke_fill=(0, 0, 0))
        draw.text(xy=(120, 40), text=str(attack), fill=(0, 255, 0), font=font, stroke_width=1, stroke_fill=(0, 0, 0))
        ...
        defense_bar = await Bar.get_bar(act_val=defense_prog, max_val=100)
        draw.text(xy=(24, 72), text="DEFENSE", fill=(255, 255, 255), font=font, stroke_width=1, stroke_fill=(0, 0, 0))
        draw.text(xy=(120, 72), text=str(defense), fill=(0, 255, 0), font=font, stroke_width=1, stroke_fill=(0, 0, 0))
        ...
        health_bar = await Bar.get_bar(act_val=health_prog, max_val=100)
        draw.text(xy=(24, 104), text="HEALTH", fill=(255, 255, 255), font=font, stroke_width=1, stroke_fill=(0, 0, 0))
        draw.text(xy=(120, 104), text=str(health), fill=(0, 255, 0), font=font, stroke_width=1, stroke_fill=(0, 0, 0))
