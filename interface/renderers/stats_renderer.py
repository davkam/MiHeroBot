...
class StatsRenderer(Renderer):
    ...
    async def render_stats(self) -> None:
        stats_image = Image.open(fp=StatsRenderer.STATS_TEMP)
        ...
        stats_image.paste(im=char, box=(48, 24), mask=char)
        stats_image.paste(im=name, box=(354, 32), mask=name)
        stats_image.paste(im=equipments, box=(352, 112), mask=equipments) 
        stats_image.paste(im=stats, box=(32, 304), mask=stats)

        self.image = stats_image
